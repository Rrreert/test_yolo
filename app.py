...
st.markdown("<h1 style='text-align: center'><img width='50' src='https://picture.gptkong.com/20241114/1411fce2b0fd9942d582bd85472990b639.jpg'></img>在线检测平台</h1>", unsafe_allow_html=True)
with st.sidebar:
    ...
    selected = option_menu("功能列表", ["首页", "图片检测", "视频检测", "摄像头检测", "历史检测", "可视化模块"], 
                           icons=['house-gear', 'image', 'film', 'webcam', 'hourglass-split', 'graph-up'], 
                           menu_icon="list-stars", default_index=0)

def predict_image():
    results = st.session_state[model_select].predict(source='predict_img.jpg', **{'save':False, 'show_conf':True}, stream=True)
    # 绘制检测结果
        ...
        with st.spinner("检测中，请稍等..."):
            ...
            while cap.isOpened():
                ...
                if frame_count % frame_rate_divider == 0:
                    results = st.session_state[model_select].predict(source=frame, **{'save':False, 'show_conf':False}, stream=True)
...
if selected == '首页':
    st.subheader("本平台是基于yolov8算法对图片、视频和摄像头三种输入实现对橘子进行检测，图片与视频检测功能支持文件上传或URL输入形式进行检测，每次检测的结果都可在历史检测中查看")
    ...
    while run:
        ...
        count_placeholder.markdown("<h4 style='text-align: center'>检测个数: {}</h4>".format(result.boxes.xyxy.cpu().numpy().shape[0]), unsafe_allow_html=True)
    ...
    f.close()
    show_info = [_.split('\t') for _ in res]
    options = [":material/movie:视频可视化", ":material/image:摄像头可视化"]
    selection = st.pills("请选择浏览的可视化类别", options, default=":material/movie:视频可视化")
    if selection == ':material/movie:视频可视化':
        detail = [_ for _ in show_info if 'video' in _[0]]
        for item in detail:
            ...
            with col5:
                ...
                st.pyplot(fig)
    elif selection == ':material/image:摄像头可视化':
